...
function SelectItem({ filters, name, open, index }) {
  ...
  const baseStyle = classNames(
    styles.filters__select,
    `filters__select_${index}`
  );
  return (
    <div className={!open ? baseStyle : styles.filters__select}>
      <h2 className={styles.filters__select_title}>{name}</h2>
      <Select
