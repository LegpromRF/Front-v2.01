...
export const handleAdminBids = createAsyncThunk(
  "orders/handleAdminBids",
  async (_, thunkAPI) => {
    const size = 40;
    const page = thunkAPI.getState().orders.adminBidsPageNumber;
    const query = Object.entries(thunkAPI.getState().orders.query)
      .filter(([name, value]) => value)
      .map(([name, value]) => `${name}=${value}&`)
      .join("");
    // console.log(`${apiEndpoints.bidAll}/?page=${page}&size=${size}`);
    const res = await axios.get(
      `${apiEndpoints.orderCards}?${query}page=${page}&size=${size}`,
      { withCredentials: true, AccessControlAllowOrigin: true }
    );
    // try {
    ...
    const stateData = {
      ...
      adminBids:
        res.data.items.map((item) => ({
          ...
          photo_urls: item.photo_urls?.split(",") || null,
        })) ?? [],
    };
    return stateData;
  }
...
export const handleBids = createAsyncThunk(
  "orders/handleBids",
  async (_, thunkAPI) => {
    const size = 40;
    ...
    // console.log(`${apiEndpoints.bidAll}/?page=${page}&size=${size}`);
    const res = await axios.get(
      `${apiEndpoints.bidAll}?${query}&page=${page}&size=${size}`,
...
export const searchBid = createAsyncThunk(
  "orders/searchBid",
  async (id, thunkAPI) => {
    if (!id) return null;
    ...
    if (isAdmin) {
      const res = await axios.get(apiEndpoints.getOrderCard(id), {
        withCredentials: true,
      });
  ...
  extraReducers: (builder) => {
    builder
      .addCase(handleAdminBids.fulfilled, (state, action) => {
        const { adminBids, adminBidsCountPages, adminBidsPageNumber } =
          action.payload;
        console.log(adminBids);
        adminBids.sort(
          (adminBidA, adminBidB) =>
            -new Date(adminBidA.created_at).getTime() +
            new Date(adminBidB.created_at).getTime()
        );
      ...
      .addCase(handleBids.fulfilled, (state, action) => {
        ...
        bids.sort(
          (bidA, bidB) =>
            -new Date(bidA.created_at).getTime() +
            new Date(bidB.created_at).getTime()
        );
        state.bids = bids;
...
export const { changeAdminBidsPage, changeBidsPage, changeStatus } =
  ordersSlice.actions;
